...
public enum PriviledgedAuthor {
    ...
    public String getReadableText(Context context) {
        if (value == PriviledgedAuthor.COMMUNITY_TA.getValue()) {
            return context.getString(tw.openedu.www.R.string.discussion_priviledged_author_label_ta);
        }

        return context.getString(tw.openedu.www.R.string.discussion_priviledged_author_label_staff);
    }
}
