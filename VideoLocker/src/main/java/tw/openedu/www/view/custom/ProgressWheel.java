...
public class ProgressWheel extends View {
    ...
    public ProgressWheel(Context context, AttributeSet attrs) {
        ...
        parseAttributes(context.obtainStyledAttributes(attrs,
                tw.openedu.www.R.styleable.ProgressWheel));
    }
    ...
    private void parseAttributes(TypedArray a) {
        barWidth = (int) a.getDimension(tw.openedu.www.R.styleable.ProgressWheel_barWidth,
                barWidth);

        rimWidth = (int) a.getDimension(tw.openedu.www.R.styleable.ProgressWheel_rimWidth,
                rimWidth);

        spinSpeed = (int) a.getDimension(tw.openedu.www.R.styleable.ProgressWheel_spinSpeed,
                spinSpeed);

        delayMillis = a.getInteger(tw.openedu.www.R.styleable.ProgressWheel_delayMillis,
                delayMillis);
        if (delayMillis < 0) {
            delayMillis = 0;
        }

        barColor = a.getColor(tw.openedu.www.R.styleable.ProgressWheel_barColor, barColor);

        barLength = (int) a.getDimension(tw.openedu.www.R.styleable.ProgressWheel_barLength,
                barLength);

        textSize = (int) a.getDimension(tw.openedu.www.R.styleable.ProgressWheel_textSize,
                textSize);

        textColor = (int) a.getColor(tw.openedu.www.R.styleable.ProgressWheel_textColor,
                textColor);

        //if the text is empty , so ignore it
        if (a.hasValue(tw.openedu.www.R.styleable.ProgressWheel_text)) {
            setText(a.getString(tw.openedu.www.R.styleable.ProgressWheel_text));
        }

        rimColor = (int) a.getColor(tw.openedu.www.R.styleable.ProgressWheel_rimColor,
                rimColor);

        circleColor = (int) a.getColor(tw.openedu.www.R.styleable.ProgressWheel_circleColor,
                circleColor);

        contourColor = a.getColor(tw.openedu.www.R.styleable.ProgressWheel_contourColor, contourColor);
        contourSize = a.getDimension(tw.openedu.www.R.styleable.ProgressWheel_contourSize, contourSize);
