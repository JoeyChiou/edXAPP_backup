...
import tw.openedu.www.model.api.AccessError;
...
import tw.openedu.www.util.images.ImageCacheManager;
...
BaseListAdapter<EnrolledCoursesResponse> {
    ...
    public MyCourseAdapter(Context context, boolean showSocial, CourseFriendsListener courseFriendsListener,IEdxEnvironment environment ) {
        super(context, tw.openedu.www.R.layout.row_course_list, environment);
    ...
    public void render(BaseViewHolder tag, final EnrolledCoursesResponse enrollment) {
            ...
            try{
                ...
                if (error == AccessError.START_DATE_ERROR) {
                    ...
                    if(type.equals(StartType.TIMESTAMP_START)){
                        startDt = getContext().getString(tw.openedu.www.R.string.label_starting_from)
                    ...
                    if(endDate!=null){
                        endDt = getContext().getString(tw.openedu.www.R.string.label_ended)
                    ...
                    if(endDate!=null){
                        endDt = getContext().getString(tw.openedu.www.R.string.label_ending_on)
                                + " - " + dateformat.format(endDate);
                        holder.starting_from.setText(endDt);
        ...
        holder.courseImage.setDefaultImageResId(tw.openedu.www.R.drawable.edx_map);
    ...
    public BaseViewHolder getTag(View convertView) {
        ...
        holder.courseTitle = (TextView) convertView
                .findViewById(tw.openedu.www.R.id.course_name);
        holder.orgCodeTv = (TextView) convertView.
                findViewById(tw.openedu.www.R.id.org_code_tv);
        holder.orSymbolTv = (TextView) convertView.
                findViewById(tw.openedu.www.R.id.or_tv);
        holder.schoolCodeTv = (TextView) convertView
                .findViewById(tw.openedu.www.R.id.school_code_tv);
        holder.starting_from = (TextView) convertView
                .findViewById(tw.openedu.www.R.id.starting_from);
        holder.courseImage = (NetworkImageView) convertView
                .findViewById(tw.openedu.www.R.id.course_image);
        holder.new_course_content = (LinearLayout) convertView
                .findViewById(tw.openedu.www.R.id.new_course_content_layout);
        holder.starting_from_layout = (LinearLayout) convertView
                .findViewById(tw.openedu.www.R.id.starting_from_layout);
        holder.certificateBanner = (RelativeLayout) convertView
                .findViewById(tw.openedu.www.R.id.course_certified_banner);
        holder.facePileView = (SocialFacePileView) convertView
                .findViewById(tw.openedu.www.R.id.course_item_facepileview);
        holder.facePileContainer = (LinearLayout) convertView
                .findViewById(tw.openedu.www.R.id.course_item_facepile_container);
        return holder;
    }
