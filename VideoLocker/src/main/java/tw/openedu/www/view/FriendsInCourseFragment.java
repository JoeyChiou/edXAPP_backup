...
import tw.openedu.www.view.adapters.FriendsInCourseAdapter;
import tw.openedu.www.view.adapters.SimpleAdapter;
import tw.openedu.www.view.custom.EButton;
...
public class FriendsInCourseFragment extends RoboFragment implements LoaderManager.LoaderCallbacks<AsyncTaskResult<List<SocialMember>>> {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View rootView = inflater.inflate(tw.openedu.www.R.layout.fragment_friends_in_course, container, false);

        ListView listView = (ListView) rootView.findViewById(tw.openedu.www.R.id.member_list);
        listView.setAdapter(adapter);

        listContainer = (SwipeRefreshLayout) rootView.findViewById(tw.openedu.www.R.id.swipe_container);
        ...
        courseLabel = (TextView) rootView.findViewById(tw.openedu.www.R.id.course_label);
        String content = ResourceUtil.getFormattedString(tw.openedu.www.R.string.friends_in_course, "courseName", courseData.getName()).toString();
        courseLabel.setText( content );

        progressBar = (ProgressBar) rootView.findViewById(tw.openedu.www.R.id.progress);
        errorLabel = (TextView) rootView.findViewById(tw.openedu.www.R.id.label_error);

        EButton courseBtn = (EButton) rootView.findViewById(tw.openedu.www.R.id.btn_open_public_course);
        ...
        if (showCourseLink) {
            ...
            courseLabel.setText(tw.openedu.www.R.string.friends_in_this_course);
    ...
    public void onLoadFinished(Loader<AsyncTaskResult<List<SocialMember>>> objectLoader, AsyncTaskResult<List<SocialMember>> result) {
        ...
        if (result.getResult() != null) {
            ...
            //TODO Handle error
            UiUtil.showMessage(getView(), getString(tw.openedu.www.R.string.error_friends_list));
        }
